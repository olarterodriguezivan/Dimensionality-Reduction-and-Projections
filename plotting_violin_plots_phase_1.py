...
SAVE_FIGURE_DIRECTORY = ROOT_DIRECTORY.joinpath("figures_violin_plots_phase_1")
...
def main( )->None:
    ...
    for function_id in FUNCTION_IDS:
        for inst_id in INSTANCE_IDS:
            for seed_lhs in DATASET_2000_CONSIDERED_SEEDS:
                for feature_name in all_feature_names:

                    print(
                        f"Plotting for feature: {feature_name}, "
                        f"function_id: {function_id}, instance_id: {inst_id}"
                        f", seed_lhs: {seed_lhs}"
                    )

                    fig, ax = plot_violin_plots_unbiased_2(
                                                        df_full_ela_2000,
