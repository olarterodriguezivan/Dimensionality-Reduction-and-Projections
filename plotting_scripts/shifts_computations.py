...
SAVE_FIGURE_DIRECTORY = ROOT_DIRECTORY.joinpath("figures_barplots")
...
def compute_problem_differences(
    df_full_2000:pd.DataFrame,
    df_full_200:pd.DataFrame,
    ...
    r"""
    Compute the differences in feature values between the full dataset and the reduced dataset.
    
    Args
    --------------
    - df_full (pd.DataFrame): The full dataset DataFrame computed in ambient space.
    - df_reduced (pd.DataFrame): The reduced dataset DataFrame computed in reduced space.
    - feature_name_list (List[str]): List of feature names to consider.
    ...
    # Join the reduced dataset with the full means and compute the differences
