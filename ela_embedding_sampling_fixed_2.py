...
def worker_extract_and_save_2(args, **kwargs):
    ...
    out_dir = (
        base_dir / "ela_features_reduced_11" / obj_type /
        f"D_{dim}" / f"s_{seed}" /
    ...
    for cur_seed in seed_list:
        ...
        feature_file = feature_dir / f"round_total.csv"

        df = extract_ela_features(cur_seed, X_reduced, fX, reduced_dim, func_id, inst_id)
...
def main():
    ...
    for key, x_file in x_dict.items():
        if key in y_dict:
            ...
            if dim < 40 and n_samples / dim and key[1] > 1999:
